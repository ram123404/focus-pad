...
export function TasksPage() {
  ...
  const allActiveTasks = useMemo(() => {
    return filterTasks(tasks.filter(t => !t.isArchived && t.status === 'todo'));
    ...
    <div className="min-h-screen p-6 lg:p-8 max-w-4xl mx-auto">
      ...
      <header className="mb-6">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {allActiveTasks.length} active task{allActiveTasks.length !== 1 ? 's' : ''}
        </p>
      </header>

      {/* Quick Add */}
      <div className="mb-6">
        <QuickAdd defaultType="task" />
      </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="today" className="mt-0">
          {renderTaskList(filteredToday, "No tasks for today. Use quick add above!")}
        </TabsContent>

        <TabsContent value="overdue" className="mt-0">
          {renderTaskList(filteredOverdue, "No overdue tasks. Great job staying on track!")}
        </TabsContent>

        <TabsContent value="upcoming" className="mt-0">
          {renderTaskList(filteredUpcoming, "No upcoming tasks scheduled.")}
        </TabsContent>

        <TabsContent value="completed" className="mt-0">
          {renderTaskList(filteredCompleted, "No completed tasks yet.")}
        </TabsContent>
      </Tabs>
    </div>
  );
